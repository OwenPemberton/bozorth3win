...
extern "C" {
	...
	int main(int argc, char** argv)
	{
		...
		strncpy_s(getopt_spec, (size_t)strlen(default_getopt_spec) + 1, default_getopt_spec, (size_t)strlen(default_getopt_spec) + 1);
#endif
		...
		if (errorfile == CNULL) {
			...
			//if ( errorfp == FPNULL ) {
			errno_t ferr = fopen_s(&errorfp, errorfile, "w");
			if (ferr != 0) {
				fprintf(stderr, "%s: ERROR: fopen() of error file \"%s\" failed.\n",
					get_progname(), errorfile);
				exit(1);
			}
		}
		...
		if (outdir != CNULL) {
			if (outfile != CNULL) {
				...
				snprintf(buf, len, "%s/%s", outdir, outfile);
				outfile = buf;
			}
			else {
		...
		if (mates_list != CNULL) {
			...
			//if ( mates_fp == FPNULL ) {
			errno_t mates_fp_err = fopen_s(&mates_fp, mates_list, "r");
			if (mates_fp_err != 0) {
				fprintf(errorfp, "%s: ERROR: fopen() of mates list file \"%s\" failed.\n",
					get_progname(), mates_list);
				exit(1);
			}
		}
		if (gallery_list != CNULL) {
			...
			errno_t gallery_fp_err = fopen_s(&gallery_fp, gallery_list, "r");
			if (gallery_fp_err != 0) {
				fprintf(errorfp, "%s: ERROR: fopen() of gallery list file \"%s\" failed.\n",
					get_progname(), gallery_list);
				exit(1);
			}
		}
		if (probe_list != CNULL) {
			...
			//if ( probe_fp == FPNULL ) {
			errno_t probe_fp_err = fopen_s(&probe_fp, probe_list, "r");
			if (probe_fp_err != 0) {
				fprintf(errorfp, "%s: ERROR: fopen() of probe list file \"%s\" failed.\n",
					get_progname(), probe_list);
				exit(1);
			}
		}
		...
		if (!no_output && outfile != CNULL) {
			...
			//if ( outfp == FPNULL ) {
			errno_t outfp_err = fopen_s(&outfp, outfile, "w");
			if (outfp_err != 0) {
				fprintf(errorfp, "%s: ERROR: fopen() of output file \"%s\" failed.\n",
					get_progname(), outfile);
				exit(1);
			}
		}
			...
			if (!threshold_set || threshold_met) {
				if (!no_output) {
					if (print_scores_at_end) {
						...
						if (scoreln == CNULL) {
							fprintf(errorfp, "%s: ERROR: strdup() for output line %d failed.\n",
								get_progname(), line_count + 1);
							exit(1);
						}
		...
		if (!no_output) {
			...
			if (fflush(outfp) != 0) {
				fprintf(errorfp, "%s: ERROR: fflush() of the match scores failed.",
					get_progname());
				++nerrors;
			}
			...
			if (outfile != CNULL) {
				...
				if (fclose(outfp) != 0) {
					fprintf(errorfp, "%s: ERROR: fclose() of output file \"%s\" failed.\n",
						get_progname(), outfile);
					++nerrors;
				}
			}
		...
		if (mates_fp != FPNULL) {
			if (fclose(mates_fp) != 0) {
				fprintf(errorfp, "%s: ERROR: fclose() of mates list \"%s\" failed.\n",
					get_progname(), mates_list);
				++nerrors;
			}
		}
		...
		if (gallery_fp != FPNULL) {
			if (fclose(gallery_fp) != 0) {
				fprintf(errorfp, "%s: ERROR: fclose() of gallery list \"%s\" failed.\n",
					get_progname(), gallery_list);
				++nerrors;
			}
		}
		...
		if (probe_fp != FPNULL) {
			if (fclose(probe_fp) != 0) {
				fprintf(errorfp, "%s: ERROR: fclose() of probe list \"%s\" failed.\n",
					get_progname(), probe_list);
				++nerrors;
			}
		}
		...
		if (errorfile != CNULL) {
			if (fclose(errorfp) != 0) {
				fprintf(errorfp, "%s: ERROR: fclose() of error file \"%s\" failed.\n",
					get_progname(), errorfile);
				++nerrors;
			}
			errorfp = FPNULL;
