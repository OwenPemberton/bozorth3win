...
#define PROGBUFFSIZE 1024
static char program_buffer[PROGBUFFSIZE];
static char * pfile;
static char * gfile;
...
{
if ( use_pid )
   snprintf( program_buffer, PROGBUFFSIZE, "%s pid %ld", basename, (long) pid );
else
   snprintf( program_buffer, PROGBUFFSIZE, "%s", basename );
}
...
outfile = malloc_or_return_error(((size_t)dirlen + (size_t)baselen + (size_t)extlen + 2), "output filename" );
if ( outfile == CNULL)
   return(CNULL);

snprintf( outfile, ((size_t)dirlen + (size_t)baselen + (size_t)extlen + 2), "%s/%s%s", outdir, basename, SCOREFILE_EXTENSION );
...
int nchars;
char * bufptr;
static char linebuf[LINEBUFFSIZE];
...
while ( *fmt ) {
   ...
   switch ( *fmt++ ) {
      case 's':
         snprintf( bufptr, LINEBUFFSIZE, "%d", n);
         break;
      case 'p':
          snprintf(bufptr, LINEBUFFSIZE, "%s", probe_file);
          break;
      case 'g':
          snprintf(bufptr, LINEBUFFSIZE, "%s", gallery_file );
         break;
      default:
         return (char *) NULL;
   }
...
if (static_flag) {
   ...
   size_t len = strnlen(linebuf, LINEBUFFSIZE) + 1;
   char *buf = malloc(len);	/* Caller must free() */
   if (buf == NULL)
   	return buf;
   //strncpy(buf, linebuf, len);
   strncpy_s(buf, len, linebuf, LINEBUFFSIZE);
   return buf;
}
}
...
struct xyt_struct * bz_load( const char * xyt_file )
{
   ...
   //fp = fopen( xyt_file, "r" );
   errt = fopen_s(&fp, xyt_file, "r");
   if ( fp == (FILE *) NULL || errt != 0)
   {
      fprintf( errorfp, "%s: ERROR: fopen() of minutiae file \"%s\" failed.\n",
                get_progname(), xyt_file);
      return XYT_NULL;
   }
   ...
   if ( fclose(fp) != 0 ) 
   {
      fprintf( errorfp, "%s: ERROR: fclose() of minutiae file \"%s\" failed.\n",
                     get_progname(), xyt_file );
      return XYT_NULL;
   }
   ...
   if ( xytq_s == XYTQ_NULL )
   {
      fprintf( errorfp, "%s: ERROR: malloc() failure while loading minutiae buffer failed.\n",
                                                     get_progname()
                                                     );
      return XYT_NULL;
   }
